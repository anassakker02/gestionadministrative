...
const BourseDetails = () => {
  ...
  const { data: bourseData, isLoading, error } = useQuery({
    queryKey: ['bourse', id],
  ...
  const handleAssignStudent = async () => {
    ...
    try {
      ...
      const response = await fetch(`/gestionadminastration/us-central1/api/v1/etudiants/${selectedStudentId}`, {
        method: 'PUT',
        headers: {
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-muted-foreground mt-2">
          {error ? 'Erreur lors du chargement de la bourse' : 'Cette bourse n\'existe pas'}
        </p>
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle className="text-2xl font-bold">{bourse.nom}</CardTitle>
            {getStatusBadge(bourse.status)}
          </div>
          <p className="text-muted-foreground mt-2">{bourse.description}</p>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            {bourse.pourcentage_remise && (
              <p><span className="font-semibold">Pourcentage de remise:</span> {bourse.pourcentage_remise}%</p>
            )}
            {bourse.montant_remise && (
              <p><span className="font-semibold">Montant de remise:</span> {bourse.montant_remise.toLocaleString()} MAD</p>
            )}
            {bourse.criteres && (
              <p className="mt-2"><span className="font-semibold">Critères:</span> {bourse.criteres}</p>
            )}
          </div>
          <div>
            {bourse.createdAt && (
              <p><span className="font-semibold">Créée le:</span> {new Date(bourse.createdAt).toLocaleDateString('fr-FR')}</p>
            )}
            {bourse.updatedAt && (
              <p className="mt-2"><span className="font-semibold">Modifiée le:</span> {new Date(bourse.updatedAt).toLocaleDateString('fr-FR')}</p>
            )}
            {bourse.isExempt && (
              <p className="mt-2"><span className="font-semibold">Exemption:</span> <Badge variant="success">Exempté</Badge></p>
            )}
          </div>
          <div className="md:col-span-2">
            ...
            <div className="w-full bg-secondary rounded-full h-2 mt-1">
              <div 
                ...
                style={{ width: `${Math.min((assignedStudents.length / 10) * 100, 100)}%` }}
              ></div>
            </div>
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
          ...
          <Button onClick={handleAssignClick} size="sm" disabled={availableStudents.length === 0}>
        ...
        <CardContent>
          {studentsLoading ? (
            ...
            <p className="text-red-600">Erreur lors du chargement des étudiants</p>
          ) : assignedStudents.length === 0 ? (
            <p className="text-muted-foreground">Aucun étudiant affecté à cette bourse pour le moment.</p>
          ) : (
