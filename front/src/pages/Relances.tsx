...
import {
  getRelancesHistorique,
  sendMessageRelance,
  recordCallRelance,
  createRelance,
  getAllRelances,
} from "@/api/relancesApi";
...
const Relances = () => {
  ...
  const updateRelanceMutation = useMutation({
    mutationFn: async (updatedRelance: Partial<Relance>) => {
      ...
      return new Promise<Relance>((resolve) => {
        setTimeout(() => {
          const currentRelances =
            queryClient.getQueryData<Relance[]>(["relances"]) || [];
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Relance mise à jour",
        description: "Les détails de l'appel ont été enregistrés.",
      });
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: `Échec de la mise à jour de la relance: ${error.message}`,
        variant: "destructive",
      });
    },
  });
  ...
  const updateRelanceStatusMutation = useMutation({
    mutationFn: async (payload: {
      id: string;
      statusRelance: Relance["statusRelance"];
      ...
      if (payload.overdueNotificationsMutedUntil) {
        ...
        updatedRelance.overdueNotificationsMutedUntil =
          payload.overdueNotificationsMutedUntil;
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["relances"] });
      toast({
        title: "Statut de relance mis à jour",
        description: "Le statut de la relance a été modifié avec succès.",
      });
    },
    onError: (error) => {
      toast({
        title: "Erreur",
        description: `Échec de la modification du statut de relance: ${error.message}`,
        variant: "destructive",
      });
    },
  });
  ...
  const sendEmailReminderMutation = useMutation({
    mutationFn: async (payload: {
      ...
      return await relanceService.sendEmailReminder(
        payload.relanceId,
        payload.to,
        payload.subject,
        payload.message,
  ...
  const sendMessageReminderMutation = useMutation({
    mutationFn: async (payload: { relanceId: string; message: string }) => {
      return await relanceService.sendMessageReminder(
        payload.relanceId,
        payload.message,
      );
    },
  ...
  const mapBackendRelanceToFrontend = (backendRelance: any): Relance => {
    ...
    return {
      ...
      statusRelance:
        backendRelance.statutEnvoi === "en attente" ? "en_attente" : "envoye",
  ...
  const { data: historiqueFromBackend = [], isLoading: isLoadingHistorique } =
    useQuery({
  ...
  const etudiantsAvecSoldeImpaye = (Array.isArray(etudiants) ? etudiants : [])
    .map((etudiant) => {
      const paiementsEtudiant = (
        Array.isArray(paiements) ? paiements : []
      ).filter((p) => p.etudiant_id === etudiant.id);
      const totalPaye = paiementsEtudiant.reduce(
  ...
  const relances = etudiantsAvecSoldeImpaye.map((etudiant, index) => ({
    ...
    priorite:
      etudiant.soldeDu > 2000 ? ("haute" as const) : ("normale" as const),
  ...
  const filteredRelances = relances.filter((relance) => {
    const matchesSearch =
      relance.etudiantNom.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesType =
      typeFilter === "all" || relance.typeRelance === typeFilter;
    const matchesPriorite =
      prioriteFilter === "all" || relance.priorite === prioriteFilter;
    const matchesStatus =
      statusFilter === "all" || relance.statusRelance === statusFilter;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "recu":
        return (
          <Badge className="bg-success text-success-foreground">Reçu</Badge>
        );
      case "appel_effectue":
    ...
    if (selectedRelance) {
      try {
        ...
        const relanceData = {
          ...
          priorite: (selectedRelance.priorite === "urgente"
            ? "haute"
            : selectedRelance.priorite) as "basse" | "normale" | "haute",
          dateCreation: new Date().toISOString().split("T")[0],
        ...
        try {
          ...
          console.log(
            "Erreur lors de la création, ajout en local:",
            createError,
          );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Suivez et gérez les relances de paiement des étudiants
          </p>
        </div>
      </div>

      <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-full sm:w-fit overflow-x-auto no-scrollbar">
        <button
          onClick={() => setActiveTab("a_relancer")}
          className={`flex-1 sm:flex-none px-6 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            ...
            {activeTab === "a_relancer" ? (
                  ...
                  {filteredRelances.map((relance) => (
                    <tr key={relance.id} className="hover:bg-gray-50">
                      <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900 italic sm:not-italic">
                          {relance.etudiantNom}
                        </div>
                        <div className="text-[10px] text-gray-500 md:hidden">
                          Retard: {relance.joursRetard}j
                        </div>
                      </td>
                      <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-4 md:px-6 py-4 hidden md:table-cell">
                        <div className="text-sm text-gray-500">
                          <span className="block">
                            Frais de scolarité {new Date().getFullYear()}
                          </span>
                          <span className="block">
                            Frais d'inscription {new Date().getFullYear()}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 md:px-6 py-4 whitespace-nowrap text-right">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              size="sm"
                              className="bg-black text-white hover:bg-gray-800 h-8 sm:h-9"
                            >
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem
                              onClick={() => handleSendMessage(relance)}
                            >
                              <MessageSquare className="h-4 w-4 mr-2" />
                              Par Message
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() => handleMakeCall(relance)}
                            >
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  ) : historiqueFromBackend.length === 0 &&
                    relancesLocales.length === 0 ? (
                    <tr>
                      <td
                        colSpan={5}
                      ...
                      >
                        ...
                        <small className="text-xs text-gray-400">
                          Debug: Backend={historiqueFromBackend.length}, Local=
                          {relancesLocales.length}
                        </small>
                      </td>
                    </tr>
                  ) : (
                    [...historiqueFromBackend, ...relancesLocales].map(
                      (relance, index) => {
                        console.log("Affichage de la relance:", relance);
                        return (
                          <tr
                            key={`${relance.id}-${index}`}
        ...
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-muted-foreground">
              Aucun étudiant avec solde impayé trouvé.
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={isCallModalOpen} onOpenChange={setIsCallModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              <p className="text-sm text-gray-700">
                ...
                <span className="font-semibold">
                  {selectedRelance?.etudiantNom}
                </span>{" "}
                (Solde: {selectedRelance?.montantDu.toLocaleString()} MAD)
              </p>
            </div>
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {updateRelanceMutation.isPending
                ? "Enregistrement..."
                : "Envoyer la relance"}
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              className="bg-black text-white hover:bg-gray-800"
            >
              {sendMessageReminderMutation.isPending
                ? "Envoi en cours..."
                : "Envoyer la relance"}
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="p-4">
            {selectedRelance ? (
              <div className="space-y-4">
                ...
                {selectedRelance.dateEnvoi && (
                  <div>
                    ...
                    <p>
                      {new Date(selectedRelance.dateEnvoi).toLocaleDateString()}
                    </p>
                  </div>
                )}
                ...
                {selectedRelance.dateAppel && (
                  <div>
                    ...
                    <p>
                      {new Date(selectedRelance.dateAppel).toLocaleDateString()}
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-primary">
                      {selectedRelance.montantDu.toLocaleString()} MAD
                    </p>
                  </div>
                </div>

                {selectedRelance.periodeCible &&
                  selectedRelance.montantPeriodeDu !== undefined && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm font-medium">
                          Période Cible
                      ...
                      <div>
                        ...
                        <p className="font-bold text-lg text-purple-600">
                          {selectedRelance.montantPeriodeDu.toLocaleString()}{" "}
                          MAD
                        </p>
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    readOnly={
                      !selectedRelance ||
                      selectedRelance.typeRelance === "email"
                    } // Allow editing for SMS and call notes
                  />
                </div>
              ...
              <p className="text-muted-foreground">
                Formulaire de création de relance à implémenter selon les
                besoins spécifiques.
              </p>
            )}
          </div>
